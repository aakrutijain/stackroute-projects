...
public class ReminderController {
	...
	public ResponseEntity<Reminder> saveReminder(@RequestBody Reminder reminder, HttpSession httpSession) {
		boolean flag= false;
		if(httpSession.getAttribute("loggedInUserId")!= null) {
			flag = reminderService.createReminder(reminder);
			if(flag) {
				return new ResponseEntity<Reminder>(reminder, HttpStatus.CREATED);
			}else {
				return new ResponseEntity<Reminder>(reminder, HttpStatus.CONFLICT);
			}
		}else {
	...
	public ResponseEntity<?> deleteReminder(@PathVariable int id, HttpSession httpSession) {
		boolean flag= false;
	...
	public ResponseEntity<?> updateReminder(@RequestBody Reminder reminder, @PathVariable int id, HttpSession httpSession) {
	...
	public ResponseEntity<?> getReminderByUserId(HttpSession httpSession) {
		...
		if(userId!= null) {
			return new ResponseEntity<>(reminderService.getAllReminderByUserId(userId),HttpStatus.OK);
		}else {
			return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
		}
	}
	...
	public ResponseEntity<?> getReminderById(@PathVariable int id, HttpSession httpSession) {
		if(httpSession.getAttribute("loggedInUserId")!= null) {
			try {
